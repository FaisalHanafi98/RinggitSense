"""
RinggitSense - Maybank statement parser
"""
import csv
import io
...
class MaybankParser(BaseParser):
    """Parser for Maybank bank statements (CSV and PDF)."""

    BANK_NAME = "Maybank"
    SUPPORTED_FORMATS = ["csv", "pdf"]

    # Common column names in Maybank CSV exports
    DATE_COLUMNS = ["date", "transaction date", "trans date", "posting date"]
    DESCRIPTION_COLUMNS = ["description", "details", "transaction details", "particulars"]
    DEBIT_COLUMNS = ["debit", "withdrawal", "debit amount", "dr"]
    CREDIT_COLUMNS = ["credit", "deposit", "credit amount", "cr"]
    BALANCE_COLUMNS = ["balance", "running balance", "available balance"]
    REFERENCE_COLUMNS = ["reference", "ref", "reference no", "ref no"]

    def parse_csv(self, content: str) -> ParserResult:
        """Parse Maybank CSV export."""
    ...
    def parse_pdf(self, content: bytes) -> ParserResult:
        ...
        return ParserResult(
            transactions=[],
            bank_name=self.BANK_NAME,
            errors=["PDF parsing requires pdfplumber library. Please export as CSV from Maybank2u."],
        )
