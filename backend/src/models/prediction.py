...
from sqlalchemy import JSON, CheckConstraint, Date, ForeignKey, Numeric, String, UniqueConstraint
...
class Prediction(Base, UUIDMixin, TimestampMixin):
    ...
    category: Mapped[str | None] = mapped_column(String(50), nullable=True)  # NULL for total

    # Prediction values
    predicted_amount: Mapped[Decimal] = mapped_column(Numeric(12, 2), nullable=False)
    confidence_low: Mapped[Decimal | None] = mapped_column(Numeric(12, 2), nullable=True)
    confidence_high: Mapped[Decimal | None] = mapped_column(Numeric(12, 2), nullable=True)
